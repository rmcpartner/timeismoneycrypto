...
#else
    const char* pszModule = "timeismoney";
#endif
    if (pex)
        return strprintf(
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\TimeIsMoney
// Windows >= Vista: C:\Users\Username\AppData\Roaming\TimeIsMoney
// Mac: ~/Library/Application Support/TimeIsMoney
// Unix: ~/.timeismoney
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TimeIsMoney";
#else
    fs::path pathRet;
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty timeismoney.conf if it does not exist
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "timeismoneyd.pid"));
