...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("TimeIsMoneyMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("TimeIsMoneyMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("TimeIsMoneyMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("timeismoney-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("TimeIsMoneyMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running TimeIsMoneyMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadTimeIsMoneyMiner() exception");
    } catch (...) {
        LogPrintf("ThreadTimeIsMoneyMiner() exception");
    }

    LogPrintf("ThreadTimeIsMoneyMiner exiting\n");
}
