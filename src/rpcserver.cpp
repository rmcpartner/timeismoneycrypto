...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop TimeIsMoney server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "TimeIsMoney server stopping";
}
...
static const CRPCCommand vRPCCommands[] =
    {
        ...
        /* TimeIsMoney features */
        {"timeismoney", "masternode", &masternode, true, true, false},
        {"timeismoney", "masternodelist", &masternodelist, true, true, false},
        {"timeismoney", "mnbudget", &mnbudget, true, true, false},
        {"timeismoney", "mnbudgetvoteraw", &mnbudgetvoteraw, true, true, false},
        {"timeismoney", "mnfinalbudget", &mnfinalbudget, true, true, false},
        {"timeismoney", "mnsync", &mnsync, true, true, false},
        {"timeismoney", "spork", &spork, true, true, false},
#ifdef ENABLE_WALLET
        {"timeismoney", "obfuscation", &obfuscation, false, false, true}, /* not threadSafe because of SendMoney */
...
void StartRPCThreads()
{
    ...
    if (((mapArgs["-rpcpassword"] == "") ||
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
                                             _("To use timeismoneyd, or the -server option to timeismoney-qt, you must set an rpcpassword in the configuration file:\n"
                                               "%s\n"
...
{
    return "> timeismoney-cli " + methodname + " " + args + "\n";
}
...
std::string HelpExampleRpc(string methodname, string args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
           "\"method\": \"" +
           methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:23008/\n";
}
