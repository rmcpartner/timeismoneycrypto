...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeTimeIsMoneyAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeTimeIsMoneyAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeTimeIsMoneyAmount = strAnonymizeTimeIsMoneyAmount.remove(strAnonymizeTimeIsMoneyAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTimeIsMoneyAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeTimeIsMoneyAmount * COIN) nMaxToAnonymize = nAnonymizeTimeIsMoneyAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeTimeIsMoneyAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeTimeIsMoneyAmount));
        strAnonymizeTimeIsMoneyAmount = strAnonymizeTimeIsMoneyAmount.remove(strAnonymizeTimeIsMoneyAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTimeIsMoneyAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
...
void OverviewPage::toggleObfuscation()
{
    ...
    if (!fEnableObfuscation) {
        ...
        if (nAnonymizeTimeIsMoneyAmount == 0) {
