...
* Update translations (ping Fuzzbawls on Slack) see [translation_process.md](https://github.com/TimeIsMoney-Project/TimeIsMoney/blob/master/doc/translation_process.md#synchronising-translations).
...
By default, Gitian will fetch source files as needed. To cache them ahead of time:
    ...
    pushd ./gitian-builder
    make -C ../timeismoney/depends download SOURCES_PATH=`pwd`/cache/common
    popd
...
NOTE: Offline builds must use the --url flag to ensure Gitian fetches only from local URLs. For example:
    ...
    pushd ./gitian-builder
    ./bin/gbuild --url timeismoney=/path/to/timeismoney,signature=/path/to/sigs {rest of arguments}
    popd
...
### Build and sign TimeIsMoney Core for Linux, Windows, and OS X:
    ...
    pushd ./gitian-builder
    ./bin/gbuild --memory 3000 --commit timeismoney=v${VERSION} ../timeismoney/contrib/gitian-descriptors/gitian-linux.yml
    ./bin/gsign --signer $SIGNER --release ${VERSION}-linux --destination ../gitian.sigs/ ../timeismoney/contrib/gitian-descriptors/gitian-linux.yml
    mv build/out/timeismoney-*.tar.gz build/out/src/timeismoney-*.tar.gz ../

    ./bin/gbuild --memory 3000 --commit timeismoney=v${VERSION} ../timeismoney/contrib/gitian-descriptors/gitian-win.yml
    ./bin/gsign --signer $SIGNER --release ${VERSION}-win-unsigned --destination ../gitian.sigs/ ../timeismoney/contrib/gitian-descriptors/gitian-win.yml
    ...
    ./bin/gbuild --memory 3000 --commit timeismoney=v${VERSION} ../timeismoney/contrib/gitian-descriptors/gitian-osx.yml
    ./bin/gsign --signer $SIGNER --release ${VERSION}-osx-unsigned --destination ../gitian.sigs/ ../timeismoney/contrib/gitian-descriptors/gitian-osx.yml
...
- Detached signatures will then be committed to the [timeismoney-detached-sigs](https://github.com/TimeIsMoney-Project/timeismoney-detached-sigs) repository, which can be combined with the unsigned apps to create signed binaries.

Create (and optionally verify) the signed OS X binary:

    pushd ./gitian-builder
    ./bin/gbuild -i --commit signature=v${VERSION} ../timeismoney/contrib/gitian-descriptors/gitian-osx-signer.yml
    ./bin/gsign --signer $SIGNER --release ${VERSION}-osx-signed --destination ../gitian.sigs/ ../timeismoney/contrib/gitian-descriptors/gitian-osx-signer.yml
...
Create (and optionally verify) the signed Windows binaries:
    ...
    pushd ./gitian-builder
    ./bin/gbuild -i --commit signature=v${VERSION} ../timeismoney/contrib/gitian-descriptors/gitian-win-signer.yml
    ./bin/gsign --signer $SIGNER --release ${VERSION}-win-signed --destination ../gitian.sigs/ ../timeismoney/contrib/gitian-descriptors/gitian-win-signer.yml
    ./bin/gverify -v -d ../gitian.sigs/ -r ${VERSION}-win-signed ../timeismoney/contrib/gitian-descriptors/gitian-win-signer.yml
...
space *do not upload these to the timeismoney.com server*.
...
- Announce the release:
  ...
  - Optionally twitter, reddit /r/timeismoney, ... but this will usually sort out itself

  - Archive release notes for the new version to `doc/release-notes/` (branch `master` and branch of the release)

  - Create a [new GitHub release](https://github.com/TimeIsMoney-Project/TimeIsMoney/releases/new) with a link to the archived release notes.
