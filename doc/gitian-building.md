...
Gitian is the deterministic build process that is used to build the TimeIsMoney
...
- [Building TimeIsMoney](#building-timeismoney)
...
./bin/gbuild --commit timeismoney=${COMMIT} --url timeismoney=${URL} ../timeismoney/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/timeismoney-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/timeismoney-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/timeismoney-build.assert
```
